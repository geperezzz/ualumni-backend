...
export class IsBefore implements ValidatorConstraintInterface {
  validate(propertyValue: any, args: ValidationArguments): boolean {
    if (!isDate(propertyValue)) {
      throw new Error(
        `The \`${args.property}\` property of \`${args.object}\` is not a Date`,
      );
    }
    const priorDate = propertyValue;
    const object = args.object as WorkExperienceDto;
    if (!object.endDate) return true;
        ...
        (constraint: {
          ...
          if ((args.object as any)[constraint.propertyName] === undefined) {
            if (!constraint.isOptional) {
              throw new Error(
                `\`${args.targetName}\` does not have a property named \`${constraint.propertyName}\``,
              );
            }
            return null;
          }
          ...
          if (!isDate(propertyValue)) {
            throw new Error(
              `The \`${constraint.propertyName}\` property of \`${args.object}\` is not a Date`,
            );
          }
  ...
  defaultMessage(args: ValidationArguments): string {
    ...
    const laterDates = args.constraints.map(
      (constraint: { propertyName: string; isOptional?: boolean }) => {
